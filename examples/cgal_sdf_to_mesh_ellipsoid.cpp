...
#include "utils/sdf_to_mesh_converter.h"
...
int main()
{
  Mesh_domain domain = Mesh_domain::create_implicit_mesh_domain(ellipsoid_function, K::Sphere_3(CGAL::ORIGIN, 2.0));
  Mesh_criteria criteria(facet_angle=30.0, facet_size=0.08, facet_distance=0.025, cell_radius_edge=2.0, cell_size=0.1);
  ...
  int instance_id = app.m_renderer->register_graphics_instance(shape_id, pos, orn, color, scaling);
