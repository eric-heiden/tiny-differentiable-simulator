...
struct Transform {
  ...
  Matrix6 matrix() const {
    ...
    // Matrix3 mErx = -Algebra::cross_matrix(translation) * Et;
#else
  ...
  Matrix6 matrix_transpose() const {
    ...
    const Matrix3 &Et = rotation;
    Matrix3 E = Algebra::transpose(rotation);
    // Matrix3 mErxT = Algebra::transpose(-Algebra::cross_matrix(translation) * Et);
#else
    const Matrix3 &E = rotation;
    Matrix3 Et = Algebra::transpose(rotation);
...
// }
  ...
  Transform operator*(const Transform &t) const {
    ...
    tr.rotation *= t.rotation;
    // tr.rotation = t.rotation * tr.rotation;
    return tr;
  }
  ...
  //    */
  //   inline RigidBodyInertia apply_transpose(const RigidBodyInertia &rbi)
  //   const {
  // #if RIGHT_ASSOCIATIVE_TRANSFORMS
  //     const Matrix3 &Et = rotation;
  //     Matrix3 E = Algebra::transpose(rotation);
